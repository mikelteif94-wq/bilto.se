...
export const UPDATE_STEPS: { key: string; label: string; icon: typeof Phone; color: string }[] = [
  { key: 'kontaktad',  label: 'Kontaktad',      icon: Phone,         color: 'text-blue-600' },
  { key: 'soker_bil',  label: 'Söker bil',      icon: Search,        color: 'text-sky-600' },
  { key: 'forhandlar', label: 'Förhandlar',     icon: Handshake,     color: 'text-amber-600' },
  { key: 'klar',      label: 'Affär klar',      icon: CheckCircle2,  color: 'text-emerald-600' },
  { key: 'avbruten',  label: 'Avbruten',       icon: XCircle,        color: 'text-slate-500' },
];

export const STEP_LABELS: Record<string, string> = Object.fromEntries(
  UPDATE_STEPS.map(s => [s.key, s.label])
);

export const STEP_META: Record<string, { label: string; color: string; bg: string; border: string; icon: typeof Phone }> = {
  ...
  avbruten:   { label: 'Avbruten',   color: 'text-slate-500',  bg: 'bg-slate-50',  border: 'border-slate-200',  icon: XCircle },
};
...
export function CustomerBookingTimeline({ bookingId }: { bookingId: string }) {
  ...
  if (loading) {
    return <div className="flex items-center gap-2 py-3 text-[13px] text-slate-400"><Loader2 className="w-4 h-4 animate-spin" /> Hämtar uppdateringar...</div>;
  }
...
export function ForhandlareBookingTimeline({ bookingId, slug }: { bookingId: string; slug: string }) {
  ...
  return (
    <div className="space-y-3">
      {/* Existing updates */}
      {loading ? (
        <div className="flex items-center gap-2 py-2 text-[13px] text-slate-400"><Loader2 className="w-4 h-4 animate-spin" /> Laddar...</div>
      ) : updates.length === 0 ? (
        <p className="text-[12px] text-slate-400 py-1">Inga uppdateringar ännu. Lägg till den första så kunden ser att du jobbar.</p>
      ) : (
      ...
      {/* Add update form */}
      {showForm ? (
        <div className="bg-[#faf8f5] rounded-lg border border-slate-200 p-3 space-y-2.5">
            ...
            {UPDATE_STEPS.map(s => {
              ...
              return (
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 h-8 px-2.5 rounded-lg text-[12px] font-medium border transition ${
                    status === s.key
          ...
          <textarea
            ...
            rows={2}
            className="w-full rounded-lg border border-slate-200 px-3 py-2 text-[13px] text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0e6efe]/20 focus:border-[#0e6efe] resize-none"
          />
          <div className="flex gap-2 justify-end">
            <button
              onClick={() => { setShowForm(false); setMessage(''); }}
              className="h-8 px-3 rounded-lg text-[12px] font-medium text-slate-500 hover:bg-slate-100 transition"
            >
              Avbryt
            </button>
            <button
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 text-[12px] font-medium transition"
        >
        ...
        {updates.map((u, idx) => {
          ...
          return (
            <div key={u.id} className="relative">
              {/* Dot */}
              <div className={`absolute -left-5 top-0.5 w-3.5 h-3.5 rounded-full border-2 border-white ${meta.bg} flex items-center justify-center ${isLast ? 'ring-2 ring-[#0e6efe]/30' : ''}`}>
              ...
              <div className="ml-1">
                ...
                {u.message && (
                  <p className="text-[13px] text-slate-600 mt-0.5 leading-relaxed">{u.message}</p>
                )}
              </div>
            </div>
          );
