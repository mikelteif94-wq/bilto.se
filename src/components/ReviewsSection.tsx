...
export default function ReviewsSection({ variant = 'light' }: Props) {
  const bg = variant === 'muted' ? 'bg-slate-50' : 'bg-[#faf8f5]';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          {REVIEWS.map((r, idx) => (
            <article
              key={idx}
              className="relative bg-[#faf8f5] rounded-xl border border-slate-200/70 p-6 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)] hover:shadow-[0_12px_30px_-10px_rgba(15,23,42,0.15)] hover:-translate-y-0.5 transition-all duration-300"
            >
        ...
        <div className="sm:hidden -mx-6">
          ...
          >
            {REVIEWS.map((r, idx) => (
              <article
                key={idx}
                className="relative bg-[#faf8f5] rounded-xl border border-slate-200/70 p-6 shadow-[0_2px_10px_-2px_rgba(15,23,42,0.06)] shrink-0 w-[85%] snap-center"
              >
                <Quote className="absolute top-5 right-5 w-6 h-6 text-slate-200" strokeWidth={2} />
                <div className="flex items-center gap-0.5 mb-4">
                  {Array.from({ length: 5 }).map((_, i) => (
