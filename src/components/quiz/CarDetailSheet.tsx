...
function OwnershipMeter({ carPrice, fuelTypes, make }: { carPrice: number; fuelTypes: string[]; make?: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide whitespace-nowrap">Ägarkostnad</span>
      <div className="flex items-center gap-[3px]">
        {[1,2,3,4,5].map(s => (
          <div key={s} className="rounded-[2px]" style={{ width: 12, height: 7, backgroundColor: s <= level ? activeColor : '#e2e8f0' }} />
        ))}
      </div>
      <span className="text-[12px] font-bold whitespace-nowrap" style={{ color: activeColor }}>{label}</span>
    </div>
  );
}
  ...
  if (step === 'start') {
    return (
      <button
        type="button"
        onClick={() => setStep('car')}
        className="w-full flex items-center gap-3 bg-emerald-600 hover:bg-emerald-700 rounded-xl px-4 py-3.5 transition-all duration-150 group text-left active:scale-[0.99]"
      >
          ...
          <div className="mx-4 mb-4">
            {loading ? (
              ...
              <motion.div
                ...
                className="w-full bg-gradient-to-b from-slate-50 to-white rounded-xl overflow-hidden flex items-center justify-center"
                style={{ height: '180px' }}
              >
                <img
                  src={heroImage}
...
function LoadingSkeleton() {
  return (
    <div className="space-y-4">
      ...
      <div className="relative h-14 rounded-xl bg-slate-100 overflow-hidden">
        <div className="absolute inset-0 -translate-x-full animate-shimmer bg-gradient-to-r from-transparent via-white/60 to-transparent" />
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2.5">
        {onSelect && (
          <button
            type="button"
            onClick={onSelect}
            className="w-full flex flex-col items-center gap-0.5 py-3.5 rounded-xl bg-[#0e6efe] hover:bg-[#0a57cc] active:scale-[0.99] text-white transition-all duration-200 shadow-lg shadow-[#0e6efe]/25"
          >
        ...
        {onFitQuiz && (
          <button
            type="button"
            onClick={onFitQuiz}
            className="w-full flex items-center justify-center gap-2 h-12 rounded-xl border border-slate-200 bg-[#faf8f5] text-slate-700 hover:bg-slate-100 text-[12px] font-semibold transition-all duration-150 active:scale-[0.97]"
          >
      ...
      <section>
        ...
        {/* EV specs panel */}
        {data.ev_specs && (
          <div className="mt-3 rounded-xl overflow-hidden border border-sky-200 bg-sky-50">
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-2.5">
        ...
        {onFitQuiz && (
          <button type="button" onClick={onFitQuiz} className="w-full flex items-center justify-center gap-2 h-12 rounded-xl border border-slate-200 bg-[#faf8f5] text-slate-700 hover:bg-slate-100 text-[12px] font-semibold transition-all duration-150 active:scale-[0.97]">
