  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] sm:aspect-[16/9] bg-gradient-to-b from-slate-50 to-slate-100 overflow-hidden">
        {imageUrl ? (
          <img src={imageUrl} alt={name} loading="lazy" className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-500 ease-out" />
        ) : (
      ...
      <div className="px-3 pt-2.5 pb-2">
          ...
          <div className="mt-1.5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between gap-1">
              <div className="flex items-baseline gap-1 min-w-0">
                ...
                <span className="text-[10px] font-semibold text-[#0e6efe]/70 shrink-0">kr/mån</span>
              </div>
              <div className="relative shrink-0">
                <button type="button" onClick={() => setShowInfo(v => !v)} className="text-slate-400 hover:text-slate-600 transition-colors">
        ...
        {monthlySaving != null && monthlySaving > 0 && (
          <p className="mt-0.5 text-[11px] font-semibold text-emerald-600">Du sparar {formatSEK(monthlySaving)} kr/mån</p>
        )}
        {equityFreed != null && equityFreed > 0 && (
          <p className="mt-0.5 text-[11px] font-semibold text-emerald-600">+{formatSEK(equityFreed)} kr frigörs vid byte</p>
        )}
        {expertComment && (
          <p className="hidden sm:block mt-1 text-[11px] text-slate-400 leading-snug line-clamp-2 min-h-[28px]">{expertComment}</p>
        )}
        {fuelLabel && <p className="mt-1 text-[10px] text-slate-400 truncate">{fuelLabel}</p>}
      </div>
