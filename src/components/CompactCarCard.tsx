...
function ScoreBadge({ value }: { value: number }) {
  ...
  const bg = isTop ? 'rgba(5,150,105,0.08)' : isMid ? 'rgba(14,110,254,0.08)' : 'rgba(217,119,6,0.08)';
  return (
    <div
  ...
  return (
    <div
      className={`group relative bg-white rounded-2xl transition-all duration-300 cursor-pointer ${
        isSelected
          ? 'ring-2 ring-bilto-500 shadow-card-active'
          : isCompared
        ...
        <div className="relative aspect-[4/3] sm:aspect-[16/9] bg-gradient-to-b from-slate-50 to-white overflow-hidden rounded-t-2xl" style={{ minHeight: 140 }}>
          {imageUrl ? (
            <img
              ...
              onError={(e) => {
                ...
                e.currentTarget.className = 'w-full h-full object-contain p-6 opacity-35';
              }}
            />
          ) : (
          ...
          {onSelect && (
            <div className={`absolute top-2.5 left-2.5 w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all duration-200 ${
              isSelected
                ? 'bg-bilto-500 border-bilto-500 shadow-md scale-110'
                : 'bg-white/85 border-slate-300 backdrop-blur-sm hover:border-bilto-400'
            }`}>
