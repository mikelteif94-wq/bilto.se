...
type Step = 'browse' | 'body' | 'fuel' | 'budget' | 'contact' | 'done';

const BUDGET_CARDS = [
  {
    id: 'under-150k',
    label: 'Under 150 000 kr',
    img: '/getImage_(4).webp',
  },
  {
    id: '150-250k',
    label: '150 000 – 250 000 kr',
    img: '/getImage_(5).webp',
  },
  {
    id: '250-400k',
    label: '250 000 – 400 000 kr',
    img: '/getImage_(6).webp',
  },
  {
    id: '400-600k',
    label: '400 000 – 600 000 kr',
    img: '/getImage_(7).webp',
  },
  {
    id: '600-800k',
    label: '600 000 – 800 000 kr',
    img: '/getImage_(8).webp',
  },
  {
    id: '800k+',
    label: 'Över 800 000 kr',
    img: '/getImage_(9).webp',
  },
];
...
export default function BilmatchPage({ onBack }: BilmatchPageProps) {
  ...
  const quizSteps: Step[] = ['body', 'fuel', 'budget', 'contact'];
  const stepIndex = quizSteps.indexOf(step);
  const totalSteps = 4;

  function handleBudgetCardClick(cardId: string) {
    const map: Record<string, string> = {
      'under-150k': '0-150000',
      '150-250k': '150000-250000',
      '250-400k': '250000-400000',
      '400-600k': '400000-600000',
      '600-800k': '600000-800000',
      '800k+': '800000+',
    };
    setBudget(map[cardId] ?? cardId);
    setStep('body');
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const fuelLabel = FUEL_TYPES.find(f => f.id === fuelType)?.label ?? fuelType;
      await supabase.from('leads').insert({
        name: name.trim(),
        phone,
        source: 'bilmatch',
        notes: `Karosseri: ${bodyLabel} | Drivmedel: ${fuelLabel} | Budget: ${budgetLabel}`,
      });
    } finally {
      setSubmitting(false);
      setStep('done');
  ...
  function goBack() {
    if (step === 'browse') { onBack(); return; }
    if (step === 'body') { setStep('browse'); return; }
    if (step === 'fuel') { setStep('body'); return; }
    if (step === 'budget') { setStep('fuel'); return; }
    if (step === 'contact') { setStep('budget'); return; }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="sticky top-0 z-20 bg-white border-b border-slate-100 px-4 h-14 flex items-center gap-3">
        ...
        <div className="flex-1">
          <p className="text-[13px] font-bold text-slate-900">
            {step === 'browse' ? 'Bilmatch' : 'Bilmatch'}
          </p>
        </div>
        {step !== 'browse' && (
          <span className="text-[12px] font-medium text-slate-400">{stepIndex + 1}/{totalSteps}</span>
        )}
      </header>
      ...
      {step === 'browse' && (
        <div className="flex-1">
          {/* Hero */}
          <div className="bg-[#0e6efe] px-5 pt-8 pb-10 text-center">
            <h1 className="text-white text-[28px] font-extrabold leading-tight tracking-tight mb-2">
              Hitta din bil match
            </h1>
            <p className="text-white/80 text-[14px] max-w-xs mx-auto leading-relaxed">
              Vi förhandlar priset åt dig. Välj budget eller låt oss matcha dig.
            </p>
          </div>

          {/* Browse by budget */}
          <div className="px-4 pt-6 pb-4">
            <h2 className="text-[22px] font-extrabold text-slate-900 tracking-tight uppercase mb-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {BUDGET_CARDS.map((card) => (
                <button
                  key={card.id}
                  type="button"
                  onClick={() => handleBudgetCardClick(card.id)}
                  className="group rounded-2xl overflow-hidden bg-[#eef0f2] shadow-sm active:scale-[0.97] transition-transform"
                >
                  <div className="aspect-[4/3] overflow-hidden flex items-end justify-center bg-[#eef0f2]">
                    <img
                      src={card.img}
                      alt={card.label}
                  ...
                  <div className="px-3 py-3">
                    <span className="text-[14px] font-bold text-slate-900 leading-snug">{card.label}</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          {/* CTA to start quiz without budget */}
          <div className="px-4 py-4">
            <div className="bg-[#0e6efe]/8 rounded-2xl p-4 flex items-center justify-between gap-3">
              <div>
                <p className="text-[13px] font-bold text-slate-900 mb-0.5">Vet du inte din budget?</p>
                <p className="text-[12px] text-slate-500">Vi hjälper dig hitta rätt ändå.</p>
              </div>
              <button
                type="button"
                onClick={() => setStep('body')}
                className="shrink-0 h-9 px-4 rounded-full bg-[#0e6efe] text-white font-bold text-[12px] hover:bg-[#0a57cc] transition inline-flex items-center gap-1.5"
              >
                Starta <ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>
          </div>
        </div>
      ...
      {step !== 'browse' && step !== 'done' && (
        <div className="flex-1 flex flex-col max-w-lg mx-auto w-full px-5 py-8">
              ...
              <div className="flex flex-col gap-2.5">
                {FUEL_TYPES.map(({ id, label, icon: Icon }) => (
                  <button
                    key={id}
                    type="button"
                    onClick={() => { setFuelType(id); setStep(budget ? 'contact' : 'budget'); }}
          ...
          {/* Step: Budget (skipped if already set via card) */}
          {step === 'budget' && (
            <div className="flex flex-col flex-1">
            ...
            <div className="flex flex-col flex-1">
              ...
              <div className="bg-slate-100 rounded-xl p-4 mb-6 space-y-1.5">
                ...
                <div className="flex items-center gap-2 text-[13px] text-slate-600">
                  ...
                  <span><span className="font-semibold text-slate-800">Budget:</span> {BUDGETS.find(b => b.id === budget)?.label ?? budget}</span>
                </div>
              </div>
