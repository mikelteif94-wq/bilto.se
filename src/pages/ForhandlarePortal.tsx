...
import { LayoutDashboard, Star, MessageSquare, Settings, LogOut, TrendingUp, Calendar, Users, Award, ChevronRight, Loader2, User, MapPin, Globe, Briefcase, CreditCard as Edit3, Check, X, Linkedin } from 'lucide-react';
...
type Tab = 'dashboard' | 'profil' | 'installningar';
    ...
    <div className="min-h-screen bg-[#f6f7f9] flex">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <main className="flex-1 px-4 sm:px-8 py-8">
          {activeTab === 'dashboard' && <Dashboard profil={profil} />}
...
function Dashboard({ profil }: { profil: ForhandlareProfil }) {
  const stats = [
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <h3 className="text-[15px] font-semibold text-slate-900 mb-1">Senaste konsultationer</h3>
        <p className="text-[13px] text-slate-400">Ingen aktivitet ännu. Leads syns här när de tilldelas dig.</p>
        <div className="mt-4 flex items-center gap-2 text-[13px] text-slate-400">
          <MessageSquare className="w-4 h-4" />
          <span>Konsultationsbokningar visas här</span>
        </div>
      </div>
    </div>
  );
