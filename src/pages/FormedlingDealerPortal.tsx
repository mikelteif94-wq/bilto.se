...
export default function FormedlingDealerPortal() {
  const [view, setView] = useState<DealerView>('overview');
  const [selectedOpportunity, setSelectedOpportunity] = useState(DEMO_OPPORTUNITIES[0]);
  const [menuOpen, setMenuOpen] = useState(false);
  const [offerSent, setOfferSent] = useState(false);
  const [offer, setOffer] = useState({ salePrice: '379000', commission: '14900', days: '14–21 dagar', storage: 'dealer' });
      ...
      <div className="flex min-h-screen">
        <aside className={`fixed lg:sticky top-0 left-0 z-40 h-screen w-64 bg-white border-r border-slate-200 flex flex-col transition-transform duration-300 ${menuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
          ...
          <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            <SidebarItem icon={LayoutDashboard} label="Översikt" active={view === 'overview'} onClick={() => navigate('overview')} />
            <SidebarItem icon={Zap} label="Nya möjligheter" badge="12" active={view === 'opportunities' || view === 'opportunity-detail' || view === 'offer-form'} onClick={() => navigate('opportunities')} />
            <SidebarItem icon={FileText} label="Mina erbjudanden" active={false} onClick={() => navigate('opportunities')} />
        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="max-w-7xl mx-auto p-5 lg:p-8">
            ...
            {view === 'opportunities' && <Opportunities onOpen={(op) => { setSelectedOpportunity(op); navigate('opportunity-detail'); }} />}
            {view === 'opportunity-detail' && <OpportunityDetail opportunity={selectedOpportunity} onBack={() => navigate('opportunities')} onOffer={() => navigate('offer-form')} />}
            {view === 'offer-form' && <OfferForm offer={offer} setOffer={setOffer} sent={offerSent} onBack={() => navigate('opportunity-detail')} onSubmit={() => setOfferSent(true)} />}
            {view === 'assignments' && <Assignments />}
            {view === 'sold' && <Sold />}
            {view === 'stats' && <Stats />}
            {view === 'profile' && <Profile />}
            {view === 'network' && <NetworkTeaser />}
          </div>
        </main>
      </div>
    </div>
...
function Overview({ onNavigate }: { onNavigate: (v: DealerView) => void }) {
  const metrics = [
    { label: 'Nya möjligheter', value: DEMO_DEALER_STATS.newOpportunities, icon: Zap, color: 'text-bilto-600', bg: 'bg-bilto-50' },
    { label: 'Aktiva erbjudanden', value: DEMO_DEALER_STATS.activeOffers, icon: FileText, color: 'text-amber-600', bg: 'bg-amber-50' },
    { label: 'Aktiva uppdrag', value: DEMO_DEALER_STATS.activeAssignments, icon: Handshake, color: 'text-emerald-600', bg: 'bg-emerald-50' },
